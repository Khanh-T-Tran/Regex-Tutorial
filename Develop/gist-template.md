...
##### Examples: 
    /^#?([a-f0-9]{6}|[a-f0-9]{3})$/
    For our hex value regular expression we have ([a-f0-9]{6}|[a-f0-9]{3}). The or operator separating these 2 components. This means that our hex value could either be 6 characters [a-f0-9]{6} or 3 characters [a-f0-9]{3}.

### Character Classes
##### Character classes tells us what type of characters to expect.
##### Examples: 
    /^#?([a-f0-9]{6}|[a-f0-9]{3})$/
    In this example character classes are confined within brackets []. The 2 character classes: [a-f0-9] and [a-f0-9] whish search for the same values. a-f searches for letters a-f and 0-9 searches for digits 0-9.
...
##### Examples: 
    /^#?([a-f0-9]{6}|[a-f0-9]{3})$/
    In this example we have ? which signifies lazy quantifier. This is referred to a lazy quantifier because it causes the regular expression engine to match as few occurances as possible. We can simply turn this lazy match into a greedy one by adding a ?.
